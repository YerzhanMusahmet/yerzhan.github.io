...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->rowCount() > 0) {
        $error = "Бұл пошта бұрын тіркелген!";
    } else {
        $sql = "INSERT INTO users (name, email, password) VALUES (?, ?, ?)";
        $pdo->prepare($sql)->execute([$name, $email, $password]);
        $_SESSION['success'] = "Сәтті тіркелдіңіз! Енді жүйеге кіріңіз.";
...
<head>
    ...
    <title>Тіркелу - Baspana Smart</title>
...
<body>
    <div class="auth-card">
        ...
        <div class="subtitle">Жаңа аккаунт ашу</div>
        
        <?php if(isset($error)) echo "<div class='error-msg'><i class='fas fa-exclamation-circle'></i> $error</div>"; ?>
        
        <form method="POST">
            <div class="input-group">
                <i class="fas fa-user"></i>
                <input type="text" name="name" placeholder="Толық аты-жөніңіз" required>
            </div>
            <div class="input-group">
            ...
            <div class="input-group">
                ...
                <input type="password" name="password" placeholder="Құпиясөз ойлап табыңыз" required>
            </div>
            <button type="submit">Тіркелу <i class="fas fa-user-plus" style="margin-left: 8px;"></i></button>
        </form>
